...
def eval_ner(output_dir: str, execute_date: str, few_shot: bool = True):
    ...
    df_original_list = []
    df_output_list = []
    for original, output in td.tqdm(zip(original_files, output_files), total = len(original_files), desc = "Evaluate NER performance", unit = "files"):
...
def eval_re(output_dir, execute_date = None, few_shot: bool = True):
    ...
    df_original_list = []
    df_output_list = []
    for original, output in td.tqdm(zip(original_files, output_files), total=len(original_files), desc="Evaluate tRE performance", unit="files"):
        with open(original, 'r') as f:
            gold = f.read()
        with open(output, 'r') as f:
            gpt_output = f.read()
